...
class AboutRegex(Koan):
    ...
    def test_matching_literal_text_not_case_sensitivity(self):
        ...
        self.assertEqual(len(re.findall('felix', string, flags=re.IGNORECASE)), 2)
        self.assertEqual(len(re.findall('felix', string, flags=re.IGNORECASE)), 2)
    ...
    def test_matching_any_character(self):
        ...
        change_this_search_string = r'(.a\d)' # <-- I want to find all uses of myArray
        #test = len(re.findall(change_this_search_string, string))
